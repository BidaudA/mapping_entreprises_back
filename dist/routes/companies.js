...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)('DELETE FROM companies WHERE id = $1', [id]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        await (0, database_1.query)('DELETE FROM company_technologies WHERE company_id = $1', [id]);
        const insertTechnology = async (technology, type) => {
            ...
            if (techResult.rows.length > 0) {
                await (0, database_1.query)('INSERT INTO company_technologies (company_id, technology_id) VALUES ($1, $2)', [id, techResult.rows[0].id]);
            }
        };
...
router.post('/', async (req, res) => {
    try {
        ...
        const insertTechnology = async (technology, type) => {
            const techResult = await (0, database_1.query)('SELECT id FROM technologies WHERE name = $1 AND type = $2', [technology, type]);
            if (techResult.rows.length > 0) {
                await (0, database_1.query)('INSERT INTO company_technologies (company_id, technology_id) VALUES ($1, $2)', [companyId, techResult.rows[0].id]);
            }
        };
        for (const tech of technologies_back) {
            await insertTechnology(tech, 'Backend');
